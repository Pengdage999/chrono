...
double fps = 60;
const std::string out_dir = GetChronoOutputPath() + "ISO_RIDETOOL_DEMO";
...
int main(int argc, char* argv[]) {
    ...
    std::string crg_road_file = "terrain/crg_roads/iso_rms10mm.crg";
    bool yup = false;

    cli.AddOption<std::string>("Demo", "m,model", "Controller model type - PID, STANLEY, SR", "STANLEY");
    ...
    GetLog() << "rideDataFileName = " << rideDataFileName << "\n";
    size_t pos_us = rideDataFileName.find_last_of("_"); // find last underscore
    std::string rmsvstr = rideDataFileName.substr(pos_us+4);
    rmsvstr.erase(rmsvstr.end()-2,rmsvstr.end()); // remove unit string "in"
    double rmsValue = std::stod(rmsvstr)/25.4;
    GetLog() << "rmsvstr = " << rmsvstr << "   value = " << rmsValue << "\n";
    ...
    while (speed <= speed_end) {
        ...
        init_csys.pos += 0.5 * ChWorldFrame::Vertical() + ChVector<>(50, 0, 0);
        ...
        std::string winTitle = "FED Alpha ISO 8608 Ride Test (RMS = ";
    ...
    if(power_limit_reached) {
        snprintf(titleString,99,"FED Alpha ISO 8608 Ride Test on %s inch roughness course. 6-Watt speed = %.2f miles/hour",cval,powRec.Get_y(6.0));
    } else {
        snprintf(titleString,99,"FED Alpha ISO 8608 Ride Test on %s inch roughness course. 6-Watt speed = %.2f miles/hour",cval,70.0);
    }
